...
local function findClasses(dir)
   ...
   for _ ,class in ipairs(dirs) do
      if not classToIdx[class] and class ~= '.' and class ~= '..'
                                                and class ~= '.DS_Store' then
...
local function findImages(dir, classToIdx)
   ...
   local extensionList =
      {'jpg', 'png', 'jpeg', 'JPG', 'JPEG', 'ppm', 'PPM', 'bmp', 'BMP'}
...
function M.exec(opt, cacheFile)
   ...
   if opt.validset == true then
      ...
      local nAll = trainImageClass:size(1)
      local shuffle = torch.randperm(nAll)
      local val0ImagePath = trainImagePath:index(1,shuffle:sub(1,50000):long())
      local val0ImageClass = trainImageClass:index(1,shuffle:sub(1,50000):long())
      trainImagePath = trainImagePath:index(1,shuffle:sub(50001,nAll):long())
      trainImageClass = trainImageClass:index(1,shuffle:sub(50001,nAll):long())
      ...
      info = {
         ...
         val = {
            imagePath = val0ImagePath,
            imageClass = val0ImageClass,
         },
         test = {
